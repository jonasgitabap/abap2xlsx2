CLASS /nag/cl_excel_writer_csv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .

*"* public components of class /NAG/CL_EXCEL_WRITER_CSV
...
*"* do not include other source files here!!!
  PROTECTED SECTION.
*"* private components of class /NAG/CL_EXCEL_WRITER_CSV
*"* do not include other source files here!!!
  PRIVATE SECTION.
    ...
    CLASS-DATA worksheet_name TYPE /nag/excel_worksheets_name .
    CLASS-DATA worksheet_index TYPE /nag/excel_active_worksheet .
...
CLASS /nag/cl_excel_writer_csv IMPLEMENTATION.
...
* End of insertion # issue 1134 - Dateretention of cellstyles(issue #139)
  ...
  METHOD create_csv.
    ...
    DATA: ls_numfmt TYPE /nag/excel_s_style_numfmt,
...
* --- Retrieve supported cell format
    SELECT * INTO CORRESPONDING FIELDS OF TABLE lt_format
      FROM seocompodf
     WHERE clsname  = '/NAG/CL_EXCEL_STYLE_NUMBER_FORMAT'
       AND typtype  = 1
       AND type     = '/NAG/EXCEL_NUMBER_FORMAT'.
...
* STEP 1: Collect strings from the first worksheet
    ...
    DATA: current_worksheet_title TYPE /nag/excel_sheet_title.
...
* --- Add empty rows
      WHILE lv_row < <fs_sheet_content>-cell_row.
        CONCATENATE lv_string /nag/cl_excel_writer_csv=>eol INTO lv_string.
        lv_row = lv_row + 1.
        lv_col = 1.
      ENDWHILE.
...
* --- Add empty columns
      WHILE lv_col < <fs_sheet_content>-cell_column.
        CONCATENATE lv_string /nag/cl_excel_writer_csv=>delimiter INTO lv_string.
        lv_col = lv_col + 1.
      ENDWHILE.
...
* ----- Use format to determine the data type and display format.
      CASE <fs_sheet_content>-data_type.
        ...
        WHEN 'd' OR 'D'.
          lc_value = /nag/cl_excel_common=>excel_string_to_date( ip_value = <fs_sheet_content>-cell_value ).
          TRY.
              lv_date = lc_value.
        ...
        WHEN 't' OR 'T'.
          lc_value = /nag/cl_excel_common=>excel_string_to_time( ip_value = <fs_sheet_content>-cell_value ).
      ...
      CONCATENATE /nag/cl_excel_writer_csv=>enclosure /nag/cl_excel_writer_csv=>enclosure INTO lv_tmp.
      CONDENSE lv_tmp.
      REPLACE ALL OCCURRENCES OF /nag/cl_excel_writer_csv=>enclosure IN lc_value WITH lv_tmp.

      FIND FIRST OCCURRENCE OF /nag/cl_excel_writer_csv=>delimiter IN lc_value.
      IF sy-subrc = 0.
        CONCATENATE lv_string /nag/cl_excel_writer_csv=>enclosure lc_value /nag/cl_excel_writer_csv=>enclosure INTO lv_string.
      ELSE.
  ...
  METHOD set_enclosure.
    /nag/cl_excel_writer_csv=>enclosure = ip_value.
  ENDMETHOD.


  METHOD set_endofline.
    /nag/cl_excel_writer_csv=>eol = ip_value.
  ENDMETHOD.
