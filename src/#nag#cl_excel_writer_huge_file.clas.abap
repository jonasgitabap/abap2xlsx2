CLASS /nag/cl_excel_writer_huge_file DEFINITION
  PUBLIC
  INHERITING FROM /nag/cl_excel_writer_2007
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    DATA worksheet TYPE REF TO /nag/cl_excel_worksheet .
ENDCLASS.



CLASS /nag/cl_excel_writer_huge_file IMPLEMENTATION.
...
** Constant node name
    ...
    DATA:
      ...
      lt_cell_data      TYPE /nag/excel_t_cell_data_unsorted,
      ls_shared_string  TYPE /nag/excel_s_shared_string,
      lv_sytabix        TYPE i,
      lt_shared_strings TYPE TABLE OF /nag/excel_s_shared_string.

    FIELD-SYMBOLS:
      <sheet_content>     TYPE /nag/excel_s_cell_data.
...
* STEP 2: Create XML
    ...
    CALL TRANSFORMATION /nag/excel_tr_shared_strings
      SOURCE root = ls_root
...
*
* Build Sheet#.xml with Simple Transformation /NAG/EXCEL_TR_SHEET
*
* This is an adaption of /NAG/CL_EXCEL_WRITER_2007.
    ...
    DATA:
      BEGIN OF l_worksheet,
        ...
        BEGIN OF pagemargins,
          left   TYPE /nag/excel_dec_8_2,
          right  TYPE /nag/excel_dec_8_2,
          top    TYPE /nag/excel_dec_8_2,
          bottom TYPE /nag/excel_dec_8_2,
          header TYPE /nag/excel_dec_8_2,
          footer TYPE /nag/excel_dec_8_2,
        END OF pagemargins,
        BEGIN OF pagesetup,
...
*
    DATA:
      ...
      lo_row_default              TYPE REF TO /nag/cl_excel_row,
      ...
      lts_sorted_columns          TYPE SORTED TABLE OF /nag/excel_cell_column
...
*
    ...
    l_worksheet-summarybelow  = io_worksheet->/nag/if_excel_sheet_properties~summarybelow.
    l_worksheet-summaryright  = io_worksheet->/nag/if_excel_sheet_properties~summaryright.
    ...
    IF io_worksheet->/nag/if_excel_sheet_properties~show_zeros EQ abap_true.
    ...
    IF iv_active = abap_true
      OR io_worksheet->/nag/if_excel_sheet_properties~selected EQ abap_true.
      l_worksheet-tabselected = lc_one.
    ELSE.
      l_worksheet-tabselected = lc_zero.
    ENDIF.

    IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale GT 400.
      io_worksheet->/nag/if_excel_sheet_properties~zoomscale = 400.
    ELSEIF io_worksheet->/nag/if_excel_sheet_properties~zoomscale LT 10.
      io_worksheet->/nag/if_excel_sheet_properties~zoomscale = 10.
    ENDIF.
    l_worksheet-zoomscale = io_worksheet->/nag/if_excel_sheet_properties~zoomscale.

    IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal NE 0.
      IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal GT 400.
        io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal = 400.
      ELSEIF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal LT 10.
        io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal = 10.
      ENDIF.
      l_worksheet-zoomscalenormal = io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal.
    ENDIF.

    IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview NE 0.
      IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview GT 400.
        io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview = 400.
      ELSEIF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview LT 10.
        io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview = 10.
      ENDIF.
      l_worksheet-zoomscalepageview = io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview.
    ENDIF.

    IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview NE 0.
      IF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview GT 400.
        io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview = 400.
      ELSEIF io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview LT 10.
        io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview = 10.
      ENDIF.
      l_worksheet-zoomscalesheetview = io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview.
    ENDIF.

    IF io_worksheet->/nag/if_excel_sheet_properties~get_right_to_left( ) EQ abap_true.
...
*
    ...
    IF lv_freeze_cell_row IS NOT INITIAL AND lv_freeze_cell_column IS NOT INITIAL.
      ...
      lv_freeze_cell_column_alpha = /nag/cl_excel_common=>convert_column2alpha( ip_column = lv_freeze_cell_column ).
      lv_value = /nag/cl_excel_common=>number_to_excel_string( ip_value = lv_freeze_cell_row  ).
...
* STEP 2: Create XML
    ...
    CALL TRANSFORMATION /nag/excel_tr_sheet
  ...
  METHOD get_cells.
*
* Callback method from transformation /NAG/EXCEL_TR_SHEET
*
* The method fills the data cells for each row.
* This saves memory if there are many rows.
*
    DATA:
      lv_cell_style TYPE /nag/excel_cell_style.

    FIELD-SYMBOLS:
      <cell>    TYPE ty_cell,
      <content> TYPE /nag/excel_s_cell_data,
      <style>   TYPE /nag/excel_s_styles_mapping.
