*&---------------------------------------------------------------------*
*& Report  /NAG/EXCEL_TEMPLATE_GET_TYPES
*&
*&---------------------------------------------------------------------*
*&
*&
...
DATA: go_excel           TYPE REF TO /nag/cl_excel,
      go_reader          TYPE REF TO /nag/if_excel_reader,
      go_template_filler TYPE REF TO /nag/cl_excel_fill_template,
      go_error           TYPE REF TO /nag/cx_excel.
...
START-OF-SELECTION.
  ...
  TRY.
      ...
      CREATE OBJECT go_reader TYPE /nag/cl_excel_reader_2007.
...
* load template
      ...
      go_template_filler = /nag/cl_excel_fill_template=>create( go_excel ).

      PERFORM get_types.

    CATCH /nag/cx_excel INTO go_error.
...
FORM get_types .
  ...
  IF p_normal IS INITIAL.
    ...
    <lv_res> = 'DATA'.
    APPEND INITIAL LINE TO lt_res ASSIGNING <lv_res>.
    <lv_res> = ': lo_data type ref to /NAG/CL_EXCEL_TEMPLATE_DATA'.
    APPEND INITIAL LINE TO lt_res ASSIGNING <lv_res>.
    <lv_res> = '.'.

  ELSE.
    APPEND INITIAL LINE TO lt_res ASSIGNING <lv_res>.
    <lv_res> = 'DATA: lo_data type ref to /NAG/CL_EXCEL_TEMPLATE_DATA.'.
  ENDIF.
...
FORM get_type_r USING iv_sheet  TYPE /nag/excel_sheet_title
                      iv_parent TYPE i
  ...
  FIELD-SYMBOLS: <lv_buf>        TYPE text80,
                 ...
                 <ls_var>        TYPE /nag/cl_excel_fill_template=>ts_variable,
                 <ls_name_style> TYPE /nag/cl_excel_fill_template=>ts_name_style.
...
FORM load_smw0
    USING
        io_reader  TYPE REF TO /nag/if_excel_reader
        iv_w3objid TYPE w3objid
    CHANGING
        ro_excel   TYPE REF TO /nag/cl_excel
    RAISING
        /nag/cx_excel.
