CLASS /nag/cl_excel_legacy_palette DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.
*"* public components of class /NAG/CL_EXCEL_LEGACY_PALETTE
*"* do not include other source files here!!!
    ...
    METHODS get_color
      IMPORTING
        !ip_index       TYPE i
      RETURNING
        VALUE(ep_color) TYPE /nag/excel_style_color_argb
      RAISING
        /nag/cx_excel .
    METHODS get_colors
      RETURNING
        VALUE(ep_colors) TYPE /nag/excel_t_style_color_argb .
    METHODS set_color
      IMPORTING
        !ip_index TYPE i
        !ip_color TYPE /nag/excel_style_color_argb
      RAISING
        /nag/cx_excel .
  PROTECTED SECTION.
*"* protected components of class /NAG/CL_EXCEL_LEGACY_PALETTE
*"* do not include other source files here!!!
  PRIVATE SECTION.

*"* private components of class /NAG/CL_EXCEL_LEGACY_PALETTE
*"* do not include other source files here!!!
    DATA modified TYPE abap_bool VALUE abap_false. "#EC NOTEXT .  .  .  .  .  .  .  .  .  . " .
    DATA colors TYPE /nag/excel_t_style_color_argb .
ENDCLASS.



CLASS /nag/cl_excel_legacy_palette IMPLEMENTATION.
  ...
  METHOD get_color.
    ...
    IF sy-subrc <> 0.
      /nag/cx_excel=>raise_text( 'Invalid color index' ).
    ENDIF.
  ENDMETHOD.
