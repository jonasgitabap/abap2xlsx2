CLASS /nag/cl_excel_column DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .

*"* public components of class /NAG/CL_EXCEL_COLUMN
*"* do not include other source files here!!!
  PUBLIC SECTION.

    METHODS constructor
      IMPORTING
        !ip_index     TYPE /nag/excel_cell_column_alpha
    ...
    METHODS set_auto_size
      ...
      RETURNING
        VALUE(io_column) TYPE REF TO /nag/cl_excel_column .
    ...
    METHODS set_column_style_by_guid
      IMPORTING
        !ip_style_guid TYPE /nag/excel_cell_style
      RAISING
        /nag/cx_excel .
    METHODS get_column_style_guid
      RETURNING
        VALUE(ep_style_guid) TYPE /nag/excel_cell_style
      RAISING
        /nag/cx_excel .
*"* protected components of class /NAG/CL_EXCEL_COLUMN
*"* do not include other source files here!!!
  PROTECTED SECTION.
*"* private components of class /NAG/CL_EXCEL_COLUMN
*"* do not include other source files here!!!
  PRIVATE SECTION.
    ...
    DATA style_guid TYPE /nag/excel_cell_style .
...
CLASS /nag/cl_excel_column IMPLEMENTATION.


  METHOD constructor.
    me->column_index = /nag/cl_excel_common=>convert_column2int( ip_index ).
    me->width         = -1.
  ...
  METHOD get_column_style_guid.
    ...
    ELSE.
      ep_style_guid = me->worksheet->/nag/if_excel_sheet_properties~get_style( ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD set_column_style_by_guid.
    ...
    IF me->excel IS NOT BOUND.
      /nag/cx_excel=>raise_text( 'Internal error - reference to /NAG/CL_EXCEL not bound' ).
    ENDIF.
    TRY.
  ...
  METHOD set_width.
    TRY.
      ...
      CATCH cx_sy_conversion_no_number.
        /nag/cx_excel=>raise_text( 'Unable to interpret width as number' ).
    ENDTRY.
  ENDMETHOD.
