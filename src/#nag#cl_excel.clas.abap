CLASS /nag/cl_excel DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.

*"* public components of class /NAG/CL_EXCEL
*"* do not include other source files here!!!
    INTERFACES /nag/if_excel_book_properties .
    INTERFACES /nag/if_excel_book_protection .
    INTERFACES /nag/if_excel_book_vba_project .

    DATA legacy_palette TYPE REF TO /nag/cl_excel_legacy_palette READ-ONLY .
    DATA security TYPE REF TO /nag/cl_excel_security .
    DATA use_template TYPE abap_bool .
    CONSTANTS version TYPE c LENGTH 10 VALUE '7.16.0'.      "#EC NOTEXT

    METHODS add_new_autofilter
      IMPORTING
        !io_sheet            TYPE REF TO /nag/cl_excel_worksheet
      RETURNING
        VALUE(ro_autofilter) TYPE REF TO /nag/cl_excel_autofilter
      RAISING
        /nag/cx_excel .
    METHODS add_new_comment
      RETURNING
        VALUE(eo_comment) TYPE REF TO /nag/cl_excel_comment .
    METHODS add_new_drawing
      IMPORTING
        !ip_type          TYPE /nag/excel_drawing_type DEFAULT /nag/cl_excel_drawing=>type_image
        !ip_title         TYPE clike OPTIONAL
      RETURNING
        VALUE(eo_drawing) TYPE REF TO /nag/cl_excel_drawing .
    ...
    METHODS add_new_style
      IMPORTING
        ...
        !io_clone_of    TYPE REF TO /nag/cl_excel_style OPTIONAL
          PREFERRED PARAMETER !ip_guid
      RETURNING
        VALUE(eo_style) TYPE REF TO /nag/cl_excel_style .
    ...
    METHODS get_active_sheet_index
      RETURNING
        VALUE(r_active_worksheet) TYPE /nag/excel_active_worksheet .
    METHODS get_active_worksheet
      RETURNING
        VALUE(eo_worksheet) TYPE REF TO /nag/cl_excel_worksheet .
    METHODS get_autofilters_reference
      RETURNING
        VALUE(ro_autofilters) TYPE REF TO /nag/cl_excel_autofilters .
    ...
    METHODS get_static_cellstyle_guid
      IMPORTING
        !ip_cstyle_complete  TYPE /nag/excel_s_cstyle_complete
        !ip_cstylex_complete TYPE /nag/excel_s_cstylex_complete
      RETURNING
    ...
    METHODS get_style_to_guid
      IMPORTING
        !ip_guid               TYPE /nag/excel_cell_style
      RETURNING
        VALUE(ep_stylemapping) TYPE /nag/excel_s_stylemapping
      RAISING
        /nag/cx_excel .
    METHODS get_theme
      EXPORTING
        !eo_theme TYPE REF TO /nag/cl_excel_theme .
    METHODS get_worksheets_iterator
      RETURNING
        VALUE(eo_iterator) TYPE REF TO /nag/cl_excel_collection_iterator .
    METHODS get_worksheets_name
      RETURNING
        VALUE(ep_name) TYPE /nag/excel_worksheets_name .
    ...
    METHODS get_worksheet_by_name
      IMPORTING
        !ip_sheet_name      TYPE /nag/excel_sheet_title
      RETURNING
        VALUE(eo_worksheet) TYPE REF TO /nag/cl_excel_worksheet .
    METHODS set_active_sheet_index
      IMPORTING
        !i_active_worksheet TYPE /nag/excel_active_worksheet
      RAISING
        /nag/cx_excel .
    METHODS set_active_sheet_index_by_name
      IMPORTING
        !i_worksheet_name TYPE /nag/excel_worksheets_name .
    ...
    METHODS set_theme
      IMPORTING
        !io_theme TYPE REF TO /nag/cl_excel_theme .
    METHODS fill_template
      IMPORTING
        !iv_data TYPE REF TO /nag/cl_excel_template_data
      RAISING
        /nag/cx_excel .
  PROTECTED SECTION.

    DATA worksheets TYPE REF TO /nag/cl_excel_worksheets .
  PRIVATE SECTION.

    DATA autofilters TYPE REF TO /nag/cl_excel_autofilters .
    DATA charts TYPE REF TO /nag/cl_excel_drawings .
    DATA default_style TYPE /nag/excel_cell_style .
*"* private components of class /NAG/CL_EXCEL
*"* do not include other source files here!!!
    DATA drawings TYPE REF TO /nag/cl_excel_drawings .
    DATA ranges TYPE REF TO /nag/cl_excel_ranges .
    DATA styles TYPE REF TO /nag/cl_excel_styles .
    DATA t_stylemapping1 TYPE /nag/excel_t_stylemapping1 .
    DATA t_stylemapping2 TYPE /nag/excel_t_stylemapping2 .
    ...
    METHODS stylemapping_dynamic_style
      IMPORTING
        !ip_style        TYPE REF TO /nag/cl_excel_style
      RETURNING
        VALUE(eo_style2) TYPE /nag/excel_s_stylemapping .
ENDCLASS.



CLASS /nag/cl_excel IMPLEMENTATION.
...
* Create default style
    ...
    DATA: style2 TYPE /nag/excel_s_stylemapping.
...
* Create default blank worksheet
  ...
  METHOD add_static_styles.
    ...
    DATA: style TYPE REF TO /nag/cl_excel_style.
    ...
    LOOP AT me->t_stylemapping1 ASSIGNING <style1> WHERE added_to_iterator IS INITIAL.
      ...
      /nag/cl_excel_common=>recursive_struct_to_class( EXPORTING i_source  = <style1>-complete_style
...
* Inizialize instance objects
    ...
    me->/nag/if_excel_book_protection~initialize( ).
    me->/nag/if_excel_book_properties~initialize( ).

    TRY.
        me->add_new_worksheet( ).
      CATCH /nag/cx_excel. " suppress syntax check error
    ...
    lo_style->fill->filltype = /nag/cl_excel_style_fill=>c_fill_pattern_gray125.
  ...
  METHOD delete_worksheet.
    ...
    DATA: lo_worksheet    TYPE REF TO /nag/cl_excel_worksheet,
          l_size          TYPE i,
  ...
  METHOD fill_template.
    ...
    DATA: lo_template_filler TYPE REF TO /nag/cl_excel_fill_template.

    FIELD-SYMBOLS:
      <lv_sheet>     TYPE /nag/excel_sheet_title,
      <lv_data_line> TYPE /nag/cl_excel_template_data=>ts_template_data_sheet.


    lo_template_filler = /nag/cl_excel_fill_template=>create( me ).
  ...
  METHOD get_static_cellstyle_guid.
    ...
    IF sy-subrc <> 0.
      ...
      style-guid = /nag/cl_excel_obsolete_func_wrap=>guid_create( ). " ins issue #379 - replacement for outdated function call
  ...
  METHOD get_style_index_in_styles.
    ...
    IF ep_index IS INITIAL.
      /nag/cx_excel=>raise_text( 'Index not found' ).
    ELSE.
  ...
  METHOD get_style_to_guid.
    ...
    IF sy-subrc <> 0.
      /nag/cx_excel=>raise_text( 'GUID not found' ).
    ENDIF.

    IF ep_stylemapping-dynamic_style_guid IS NOT INITIAL.
      lo_style = me->get_style_from_guid( ip_guid ).
      /nag/cl_excel_common=>recursive_class_to_struct( EXPORTING i_source = lo_style
  ...
  METHOD get_worksheet_by_index.
    ...
    DATA: lv_index TYPE /nag/excel_active_worksheet.
...
*--------------------------------------------------------------------*
  ...
  METHOD set_active_sheet_index_by_name.
    ...
    DATA: ws_it    TYPE REF TO /nag/cl_excel_collection_iterator,
          ws       TYPE REF TO /nag/cl_excel_worksheet,
          lv_title TYPE /nag/excel_sheet_title,
...
* dynamically
  ...
  METHOD /nag/if_excel_book_properties~initialize.
    DATA: lv_timestamp TYPE timestampl.

    me->/nag/if_excel_book_properties~application     = 'Microsoft Excel'.
    me->/nag/if_excel_book_properties~appversion      = '12.0000'.

    GET TIME STAMP FIELD lv_timestamp.
    me->/nag/if_excel_book_properties~created         = lv_timestamp.
    me->/nag/if_excel_book_properties~creator         = sy-uname.
    me->/nag/if_excel_book_properties~description     = /nag/cl_excel=>version.
    me->/nag/if_excel_book_properties~modified        = lv_timestamp.
    me->/nag/if_excel_book_properties~lastmodifiedby  = sy-uname.
  ENDMETHOD.


  METHOD /nag/if_excel_book_protection~initialize.
    me->/nag/if_excel_book_protection~protected      = /nag/if_excel_book_protection=>c_unprotected.
    me->/nag/if_excel_book_protection~lockrevision   = /nag/if_excel_book_protection=>c_unlocked.
    me->/nag/if_excel_book_protection~lockstructure  = /nag/if_excel_book_protection=>c_unlocked.
    me->/nag/if_excel_book_protection~lockwindows    = /nag/if_excel_book_protection=>c_unlocked.
    CLEAR me->/nag/if_excel_book_protection~workbookpassword.
    CLEAR me->/nag/if_excel_book_protection~revisionspassword.
  ENDMETHOD.


  METHOD /nag/if_excel_book_vba_project~set_codename.
    me->/nag/if_excel_book_vba_project~codename = ip_codename.
  ENDMETHOD.


  METHOD /nag/if_excel_book_vba_project~set_codename_pr.
    me->/nag/if_excel_book_vba_project~codename_pr = ip_codename_pr.
  ENDMETHOD.


  METHOD /nag/if_excel_book_vba_project~set_vbaproject.
    me->/nag/if_excel_book_vba_project~vbaproject = ip_vbaproject.
  ENDMETHOD.
ENDCLASS.
