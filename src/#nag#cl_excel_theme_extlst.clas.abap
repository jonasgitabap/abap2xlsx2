CLASS /nag/cl_excel_theme_extlst DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS /nag/cl_excel_theme_extlst IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF extlst IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = /nag/cl_excel_theme=>c_theme_prefix
                                                                name   = /nag/cl_excel_theme=>c_theme_extlst
                                                                parent = lo_theme ).
  ...
  METHOD load.
    extlst = /nag/cl_excel_common=>clone_ixml_with_namespaces( io_extlst ).
  ENDMETHOD.                    "load
ENDCLASS.
