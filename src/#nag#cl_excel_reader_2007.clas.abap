CLASS /nag/cl_excel_reader_2007 DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.
*"* public components of class /NAG/CL_EXCEL_READER_2007
*"* do not include other source files here!!!

    INTERFACES /nag/if_excel_reader .
  ...
  PROTECTED SECTION.
    ...
    TYPES:
*"* protected components of class /NAG/CL_EXCEL_READER_2007
*"* do not include other source files here!!!
      BEGIN OF t_relationship,
        ...
        worksheet    TYPE REF TO /nag/cl_excel_worksheet,
    ...
    TYPES:
      ...
      END OF t_range .
    TYPES:
      t_fills   TYPE STANDARD TABLE OF REF TO /nag/cl_excel_style_fill WITH NON-UNIQUE DEFAULT KEY .
    TYPES:
      t_borders TYPE STANDARD TABLE OF REF TO /nag/cl_excel_style_borders WITH NON-UNIQUE DEFAULT KEY .
    TYPES:
      t_fonts   TYPE STANDARD TABLE OF REF TO /nag/cl_excel_style_font WITH NON-UNIQUE DEFAULT KEY .
    TYPES:
      t_style_refs TYPE STANDARD TABLE OF REF TO /nag/cl_excel_style WITH NON-UNIQUE DEFAULT KEY .
    TYPES:
      BEGIN OF t_color,
        indexed TYPE string,
        rgb     TYPE string,
    ...
    TYPES:
      BEGIN OF ty_ref_formulae,
        sheet   TYPE REF TO /nag/cl_excel_worksheet,
        row     TYPE i,
        column  TYPE i,
        si      TYPE i,
        ref     TYPE string,
    ...
    DATA mt_dxf_styles TYPE /nag/excel_t_styles_cond_mapping .

    METHODS fill_row_outlines
      IMPORTING
        !io_worksheet TYPE REF TO /nag/cl_excel_worksheet
      RAISING
        /nag/cx_excel .
    ...
    METHODS load_style_font
      ...
      RETURNING
        VALUE(ro_font)  TYPE REF TO /nag/cl_excel_style_font .
    ...
    METHODS load_style_num_formats
      IMPORTING
        !ip_xml               TYPE REF TO if_ixml_document
      RETURNING
        VALUE(ep_num_formats) TYPE /nag/cl_excel_style_number_format=>t_num_formats .
    ...
    METHODS load_worksheet_cond_format_aa
      IMPORTING
        ...
        !io_style_cond TYPE REF TO /nag/cl_excel_style_cond.
    ...
    METHODS get_dxf_style_guid
      IMPORTING
        ...
        !io_excel            TYPE REF TO /nag/cl_excel
      RETURNING
        VALUE(rv_style_guid) TYPE /nag/excel_cell_style .
    METHODS load_theme
      IMPORTING
        iv_path   TYPE string
        !ip_excel TYPE REF TO /nag/cl_excel
...
CLASS /nag/cl_excel_reader_2007 IMPLEMENTATION.
  ...
  METHOD fill_row_outlines.
    ...
    DATA: lt_row_data             TYPE ltt_row_data,
          ...
          lt_outline_rows         TYPE /nag/cl_excel_worksheet=>mty_ts_outlines_row,
          ls_outline_row          LIKE LINE OF lt_outline_rows,
          lo_row                  TYPE REF TO /nag/cl_excel_row,
          lo_row_iterator         TYPE REF TO /nag/cl_excel_collection_iterator,
...
* we now need to determine whether the outline is collapsed or not
    LOOP AT lt_outline_rows INTO ls_outline_row.
      ...
      IF io_worksheet->/nag/if_excel_sheet_properties~summarybelow = /nag/if_excel_sheet_properties=>c_below_off.
...
*--------------------------------------------------------------------*
  ...
  METHOD get_dxf_style_guid.
    ...
    DATA: ls_cstyle  TYPE /nag/excel_s_cstyle_complete,
          ls_cstylex TYPE /nag/excel_s_cstylex_complete.
          ...
          IF lo_ixml_element IS BOUND.
            ...
            IF lo_ixml_element2 IS BOUND.
              ...
              IF lv_val IS NOT INITIAL.
                ls_cstyle-fill-filltype       = /nag/cl_excel_style_fill=>c_fill_solid.
...
*--------------------------------------------------------------------*
  ...
  METHOD load_drawing_anchor.
    ...
    DATA: lo_drawing     TYPE REF TO /nag/cl_excel_drawing,
          ...
          ls_upper       TYPE /nag/excel_drawing_location,
          ls_lower       TYPE /nag/excel_drawing_location,
          ls_size        TYPE /nag/excel_drawing_size,
          ext            TYPE t_ext,
          ...
          blip           TYPE t_blip,
          chart          TYPE t_chart,
          drawing_type   TYPE /nag/excel_drawing_type,
    ...
    ELSE.
      ...
      TRY.
          ls_size-width  = /nag/cl_excel_drawing=>emu2pixel( ls_size-width ).
        CATCH cx_root.
      ENDTRY.
      TRY.
          ls_size-height = /nag/cl_excel_drawing=>emu2pixel( ls_size-height ).
        CATCH cx_root.
      ENDTRY.
    ENDIF.
    ...
    IF node IS NOT INITIAL.
      ...
      drawing_type = /nag/cl_excel_drawing=>type_image.
    ENDIF.
    ...
    IF node IS NOT INITIAL.
      ...
      drawing_type = /nag/cl_excel_drawing=>type_chart.
    ENDIF.
    ...
    IF drawing_type = /nag/cl_excel_drawing=>type_chart.
...
*--------------------------------------------------------------------*
    ...
    DATA:
      ...
      lo_font               TYPE REF TO /nag/cl_excel_style_font,
      ls_rtf                TYPE /nag/excel_s_rtf,
...
*--------------------------------------------------------------------*
    ...
    DATA: lo_styles_xml                 TYPE REF TO if_ixml_document,
          lo_style                      TYPE REF TO /nag/cl_excel_style,

          lt_num_formats                TYPE /nag/cl_excel_style_number_format=>t_num_formats,
          lt_fills                      TYPE t_fills,
          lt_borders                    TYPE t_borders,
          lt_fonts                      TYPE t_fonts,

          ls_num_format                 TYPE /nag/cl_excel_style_number_format=>t_num_format,
          ls_fill                       TYPE REF TO /nag/cl_excel_style_fill,
          ls_cell_border                TYPE REF TO /nag/cl_excel_style_borders,
          ls_font                       TYPE REF TO /nag/cl_excel_style_font,
...
*--------------------------------------------------------------------*
        ...
        IF lo_node_cellxfs_xf_protection IS BOUND.
          ...
          IF ls_protection-locked = '1' OR ls_protection-locked = 'true'.
            lo_style->protection->locked = /nag/cl_excel_style_protection=>c_protection_locked.
          ELSE.
            lo_style->protection->locked = /nag/cl_excel_style_protection=>c_protection_unlocked.
          ENDIF.

          IF ls_protection-hidden = '1' OR ls_protection-hidden = 'true'.
            lo_style->protection->hidden = /nag/cl_excel_style_protection=>c_protection_hidden.
          ELSE.
            lo_style->protection->hidden = /nag/cl_excel_style_protection=>c_protection_unhidden.
          ENDIF.
...
*--------------------------------------------------------------------*
    DATA: lo_node_border      TYPE REF TO if_ixml_element,
          ...
          lo_cell_border      TYPE REF TO /nag/cl_excel_style_borders,
          lo_border           TYPE REF TO /nag/cl_excel_style_border,
...
*--------------------------------------------------------------------*
      IF lo_node_border->get_attribute( 'diagonalDown' ) IS NOT INITIAL.
        ADD /nag/cl_excel_style_borders=>c_diagonal_down TO lo_cell_border->diagonal_mode.
      ENDIF.

      IF lo_node_border->get_attribute( 'diagonalUp' ) IS NOT INITIAL.
        ADD /nag/cl_excel_style_borders=>c_diagonal_up TO lo_cell_border->diagonal_mode.
      ENDIF.
...
*--------------------------------------------------------------------*
    DATA: lv_value           TYPE string,
          ...
          lo_fill            TYPE REF TO /nag/cl_excel_style_fill,
          ls_color           TYPE t_color.
...
* §1 built-in numberformats
*--------------------------------------------------------------------*
    ep_num_formats = /nag/cl_excel_style_number_format=>mt_built_in_num_formats.
...
*--------------------------------------------------------------------*
  ...
  METHOD load_theme.
    DATA theme TYPE REF TO /nag/cl_excel_theme.
...
*--------------------------------------------------------------------*
* ToDos:
*        2do§1   Move macro-reading from /nag/cl_excel_reader_xlsm to this class
...
*--------------------------------------------------------------------*
    ...
    DATA:
      ...
      lv_worksheet_title         TYPE /nag/excel_sheet_title,
...
*--------------------------------------------------------------------*
      ...
      lv_/nag/excel_active_worksheet TYPE /nag/excel_active_worksheet,
*--------------------------------------------------------------------*
* issue#235 - repeat rows/columns  - added autofilter support while changing this section
      lo_autofilter              TYPE REF TO /nag/cl_excel_autofilter,
      ls_area                    TYPE /nag/excel_s_autofilter_area,
      lv_col_start_alpha         TYPE /nag/excel_cell_column_alpha,
      lv_col_end_alpha           TYPE /nag/excel_cell_column_alpha,
      lv_row_start               TYPE /nag/excel_cell_row,
      lv_row_end                 TYPE /nag/excel_cell_row,
      lv_regex                   TYPE string,
      lv_range_value_1           TYPE /nag/excel_range_value,
      lv_range_value_2           TYPE /nag/excel_range_value.
*--------------------------------------------------------------------*
...
*   §2.5    [VBA (Macro)]           - supported in class /nag/cl_excel_reader_xlsm but should be moved here and autodetect
*   ...
*
...
*--------------------------------------------------------------------*
      CASE ls_sheet-state.
        ...
        WHEN lcv_worksheet_state_hidden.
          lo_worksheet->/nag/if_excel_sheet_properties~hidden = /nag/if_excel_sheet_properties=>c_hidden.

        WHEN lcv_worksheet_state_veryhidden.
          lo_worksheet->/nag/if_excel_sheet_properties~hidden = /nag/if_excel_sheet_properties=>c_veryhidden.
...
*   §3.2    Active worksheet
*--------------------------------------------------------------------*
    lv_/nag/excel_active_worksheet = 1.                                 " First sheet = active sheet if nothing else specified.
    ...
    IF lo_node IS BOUND.
      ...
      TRY.
          lv_/nag/excel_active_worksheet = lv_active_sheet_string + 1.  " EXCEL numbers the sheets from 0 onwards --> index into worksheettable is increased by one
        CATCH cx_sy_conversion_error. "#EC NO_HANDLER    - error here --> just use the default 1st sheet
      ENDTRY.
    ENDIF.
    io_excel->set_active_sheet_index( lv_/nag/excel_active_worksheet ).
*--------------------------------------------------------------------*
...
* insert autofilters
*--------------------------------------------------------------------*
            WHEN /nag/cl_excel_autofilters=>c_autofilter.
              " begin Dennis Schaaf
              TRY.
                  /nag/cl_excel_common=>convert_range2column_a_row( EXPORTING i_range        = lv_range_value
                  ...
                  ls_area-col_start = /nag/cl_excel_common=>convert_column2int( lv_col_start_alpha ).
                  ls_area-col_end   = /nag/cl_excel_common=>convert_column2int( lv_col_end_alpha ).
...
*--------------------------------------------------------------------*
            WHEN /nag/if_excel_sheet_printsettings=>gcv_print_title_name.
...
* Split into two ranges if necessary
              ...
              ENDIF.
* 1st range
              /nag/cl_excel_common=>convert_range2column_a_row( EXPORTING i_range            = lv_range_value_1
              ...
              IF lv_col_start_alpha IS NOT INITIAL.
                <worksheet>-worksheet->/nag/if_excel_sheet_printsettings~set_print_repeat_columns( iv_columns_from = lv_col_start_alpha
                                                                                      iv_columns_to   = lv_col_end_alpha ).
              ENDIF.
              IF lv_row_start IS NOT INITIAL.
                <worksheet>-worksheet->/nag/if_excel_sheet_printsettings~set_print_repeat_rows( iv_rows_from = lv_row_start
                                                                                   iv_rows_to   = lv_row_end ).
              ENDIF.

* 2nd range
              /nag/cl_excel_common=>convert_range2column_a_row( EXPORTING i_range            = lv_range_value_2
                                                                      i_allow_1dim_range = abap_true
                                                            IMPORTING e_column_start     = lv_col_start_alpha
                                                                      e_column_end       = lv_col_end_alpha
                                                                      e_row_start        = lv_row_start
...
*--------------------------------------------------------------------*
    ...
    TYPES: BEGIN OF lty_sheetview,
             showgridlines            TYPE /nag/excel_show_gridlines,
    ...
    TYPES: BEGIN OF lty_datavalidation,
             type             TYPE /nag/excel_data_val_type,
             allowblank       TYPE flag,
             showinputmessage TYPE flag,
             showerrormessage TYPE flag,
             showdropdown     TYPE flag,
             operator         TYPE /nag/excel_data_val_operator,
             formula1         TYPE /nag/excel_validation_formula1,
             formula2         TYPE /nag/excel_validation_formula1,
             sqref            TYPE string,
             cell_column      TYPE /nag/excel_cell_column_alpha,
             cell_column_to   TYPE /nag/excel_cell_column_alpha,
             cell_row         TYPE /nag/excel_cell_row,
             cell_row_to      TYPE /nag/excel_cell_row,
             error            TYPE string,
             errortitle       TYPE string,
             prompt           TYPE string,
             prompttitle      TYPE string,
             errorstyle       TYPE /nag/excel_data_val_error_style,
    ...
    DATA: lo_ixml_worksheet           TYPE REF TO if_ixml_document,
          ...
          lv_cell_formula             TYPE /nag/excel_cell_formula,
          lv_cell_column              TYPE /nag/excel_cell_column_alpha,
          lv_cell_row                 TYPE /nag/excel_cell_row,
          lo_excel_style              TYPE REF TO /nag/cl_excel_style,
          ...
          ls_excel_pane               TYPE /nag/excel_pane,
          lv_pane_cell_row            TYPE /nag/excel_cell_row,
          lv_pane_cell_col_a          TYPE /nag/excel_cell_column_alpha,
          lv_pane_cell_col            TYPE /nag/excel_cell_column,
          ...
          lv_merge_column_start       TYPE /nag/excel_cell_column_alpha,
          lv_merge_column_end         TYPE /nag/excel_cell_column_alpha,
          lv_merge_row_start          TYPE /nag/excel_cell_row,
          lv_merge_row_end            TYPE /nag/excel_cell_row,
          ...
          ls_odd_header               TYPE /nag/excel_s_worksheet_head_foot,
          ...
          ls_excel_s_tabcolor         TYPE /nag/excel_s_tabcolor,
...
*---    End of current code aligning -------------------------------------------------------------------
          ...
          lo_data_validation          TYPE REF TO /nag/cl_excel_data_validation,
...
*--------------------------------------------------------------------*
    ...
    TRY.                                                                          " +#222  _rels/xxx.rels might not be present.  If not found there can be no drawings --> just ignore this section
      ...
      CATCH /nag/cx_excel.                            "#EC NO_HANDLER +#222
...
*                   in the "load_worksheet_drawing" shouldn't lead to an abortion of the reading
          TRY.
            ...
            CATCH /nag/cx_excel. "--> then ignore it
          ENDTRY.
...
* Theme not supported yet
    ...
    TRY.
        me->load_worksheet_tables( io_ixml_worksheet = lo_ixml_worksheet
                                   ...
                                   it_tables         = lt_tables ).
      CATCH /nag/cx_excel. " Ignore reading errors - pass everything we were able to identify
    ENDTRY.
...
*--------------------------------------------------------------------*
          ...
          IF ls_formula_attributes-t = 'shared'.
            /nag/cl_excel_common=>convert_columnrow2column_a_row( EXPORTING
            ...
            TRY.
                ...
                ls_ref_formula-column    = /nag/cl_excel_common=>convert_column2int( lv_cell_column ).
...
*--------------------------------------------------------------------*
    IF lv_max_col = 0.
      ...
      IF lo_ixml_imension_elem IS BOUND.
        ...
        ENDIF.
        lv_max_col = /nag/cl_excel_common=>convert_column2int( lv_dimension_range ).
      ENDIF.
    ENDIF.
*--------------------------------------------------------------------*
    ...
    WHILE lo_ixml_column_elem IS BOUND.
      ...
      IF   ls_column-customwidth   = lc_xml_attr_true
        ...
        WHILE lv_index <= ls_column-max AND lv_index <= lv_max_col.
          ...
          lv_column_alpha = /nag/cl_excel_common=>convert_column2alpha( lv_index ).
          ...
          IF ls_column-style > ''.
            ...
            DATA: dummy_/nag/excel_cell_style TYPE /nag/excel_cell_style.
            dummy_/nag/excel_cell_style = lo_excel_style->get_guid( ).
            lo_column->set_column_style_by_guid( dummy_/nag/excel_cell_style ).
          ENDIF.
...
* issue #367 - hide columns from
      IF ls_column-max = /nag/cl_excel_common=>c_excel_sheet_max_col.     " Max = very right column
        IF ls_column-hidden = 1     " all hidden
          AND ls_column-min > 0.
          io_worksheet->/nag/if_excel_sheet_properties~hide_columns_from = /nag/cl_excel_common=>convert_column2alpha( ls_column-min ).
        ELSEIF ls_column-style > ''.
          lv_index_temp = ls_column-style + 1.
          READ TABLE styles INTO lo_excel_style INDEX lv_index_temp.
          dummy_/nag/excel_cell_style = lo_excel_style->get_guid( ).
* Set style for remaining columns
          io_worksheet->/nag/if_excel_sheet_properties~set_style( dummy_/nag/excel_cell_style ).
        ENDIF.
      ENDIF.
    ...
    IF ls_sheetview-righttoleft = lc_xml_attr_true
      ...
      io_worksheet->/nag/if_excel_sheet_properties~set_right_to_left( abap_true ).
    ENDIF.
    io_worksheet->/nag/if_excel_sheet_properties~zoomscale                 = ls_sheetview-zoomscale.
    io_worksheet->/nag/if_excel_sheet_properties~zoomscale_normal          = ls_sheetview-zoomscalenormal.
    io_worksheet->/nag/if_excel_sheet_properties~zoomscale_pagelayoutview  = ls_sheetview-zoomscalepagelayoutview.
    io_worksheet->/nag/if_excel_sheet_properties~zoomscale_sheetlayoutview = ls_sheetview-zoomscalesheetlayoutview.
    ...
    WHILE lo_ixml_mergecell_elem IS BOUND.
      ...
      /nag/cl_excel_common=>convert_range2column_a_row( EXPORTING
...
*        2do§1   Header/footer
    ...
    TRY.
      ...
      CATCH /nag/cx_excel. " Ignore Hyperlink reading errors - pass everything we were able to identify
    ENDTRY.
    ...
    TRY.
        me->load_worksheet_pagebreaks( io_ixml_worksheet      = lo_ixml_worksheet
                                       io_worksheet           = io_worksheet ).
      CATCH /nag/cx_excel. " Ignore pagebreak reading errors - pass everything we were able to identify
    ENDTRY.

    TRY.
        me->load_worksheet_autofilter( io_ixml_worksheet      = lo_ixml_worksheet
                                       io_worksheet           = io_worksheet ).
      CATCH /nag/cx_excel. " Ignore autofilter reading errors - pass everything we were able to identify
    ENDTRY.

    TRY.
        me->load_worksheet_ignored_errors( io_ixml_worksheet      = lo_ixml_worksheet
                                           io_worksheet           = io_worksheet ).
      CATCH /nag/cx_excel. " Ignore "ignoredErrors" reading errors - pass everything we were able to identify
    ENDTRY.
  ...
  METHOD load_worksheet_autofilter.
    ...
    DATA: lo_ixml_autofilter_elem    TYPE REF TO if_ixml_element,
          ...
          lv_val                     TYPE string,
          lo_autofilters             TYPE REF TO /nag/cl_excel_autofilters,
          lo_autofilter              TYPE REF TO /nag/cl_excel_autofilter.
  ...
  METHOD load_worksheet_cond_format.
    ...
    DATA: lo_ixml_cond_formats TYPE REF TO if_ixml_node_collection,
          ...
          lo_style_cond2       TYPE REF TO /nag/cl_excel_style_cond.


    DATA: lv_area           TYPE string,
          lt_areas          TYPE STANDARD TABLE OF string WITH NON-UNIQUE DEFAULT KEY,
          lv_area_start_row TYPE /nag/excel_cell_row,
          lv_area_end_row   TYPE /nag/excel_cell_row,
          lv_area_start_col TYPE /nag/excel_cell_column_alpha,
          lv_area_end_col   TYPE /nag/excel_cell_column_alpha,
          lv_rule           TYPE /nag/excel_condition_rule.
...
*--------------------------------------------------------------------*
        CASE lv_rule.
          ...
          WHEN /nag/cl_excel_style_cond=>c_rule_cellis.
...
*--------------------------------------------------------------------*
          ...
          LOOP AT lt_areas INTO lv_area.
            ...
            /nag/cl_excel_common=>convert_range2column_a_row( EXPORTING i_range        = lv_area
...
*--------------------------------------------------------------------*
  ...
  METHOD load_worksheet_drawing.
    ...
    WHILE node IS BOUND.
      ...
      path2 = path.
      /nag/cl_excel_common=>split_file( EXPORTING ip_file = path2
...
* two-cell anchor ******************
  ...
  METHOD load_comments.
    DATA: lo_comments_xml       TYPE REF TO if_ixml_document,
          ...
          lo_comment            TYPE REF TO /nag/cl_excel_comment,
  ...
  METHOD load_worksheet_hyperlinks.
    ...
    DATA: lo_ixml_hyperlinks TYPE REF TO if_ixml_node_collection,
          ...
          lv_row_start       TYPE /nag/excel_cell_row,
          lv_row_end         TYPE /nag/excel_cell_row,
          lv_column_start    TYPE /nag/excel_cell_column_alpha,
          lv_column_end      TYPE /nag/excel_cell_column_alpha,
  ...
  METHOD load_worksheet_ignored_errors.
    ...
    DATA: lo_ixml_ignored_errors TYPE REF TO if_ixml_node_collection,
          ...
          lo_ixml_iterator       TYPE REF TO if_ixml_node_iterator,
          ls_ignored_error       TYPE /nag/cl_excel_worksheet=>mty_s_ignored_errors,
          lt_ignored_errors      TYPE /nag/cl_excel_worksheet=>mty_th_ignored_errors.
  ...
  METHOD load_worksheet_pagebreaks.
    ...
    DATA: lt_pagebreaks TYPE STANDARD TABLE OF /nag/cl_excel_worksheet_pagebreaks=>ts_pagebreak_at,
          lo_pagebreaks TYPE REF TO /nag/cl_excel_worksheet_pagebreaks.
...
*--------------------------------------------------------------------*
  ...
  METHOD load_worksheet_pagemargins.
    ...
    IF lo_ixml_pagemargins_elem IS NOT INITIAL.
      ...
      io_worksheet->sheet_setup->margin_bottom = /nag/cl_excel_common=>excel_string_to_number( ls_pagemargins-bottom ).
      io_worksheet->sheet_setup->margin_footer = /nag/cl_excel_common=>excel_string_to_number( ls_pagemargins-footer ).
      io_worksheet->sheet_setup->margin_header = /nag/cl_excel_common=>excel_string_to_number( ls_pagemargins-header ).
      io_worksheet->sheet_setup->margin_left   = /nag/cl_excel_common=>excel_string_to_number( ls_pagemargins-left   ).
      io_worksheet->sheet_setup->margin_right  = /nag/cl_excel_common=>excel_string_to_number( ls_pagemargins-right  ).
      io_worksheet->sheet_setup->margin_top    = /nag/cl_excel_common=>excel_string_to_number( ls_pagemargins-top    ).
    ENDIF.
  ...
  METHOD load_worksheet_tables.
    ...
    DATA ls_field_catalog TYPE /nag/excel_s_fieldcatalog.
    DATA lo_ixml_iterator TYPE REF TO if_ixml_node_iterator.
    DATA ls_table_settings TYPE /nag/excel_s_table_settings.
    ...
    FIELD-SYMBOLS <ls_field> TYPE /nag/excel_s_fieldcatalog.
...
*--------------------------------------------------------------------*
  ...
  METHOD resolve_referenced_formulae.
    ...
    DATA: ls_ref_formula       LIKE LINE OF me->mt_ref_formulae,
          ...
          lv_col_from          TYPE /nag/excel_cell_column_alpha,
          lv_col_to            TYPE /nag/excel_cell_column_alpha,
...
*--------------------------------------------------------------------*
    LOOP AT me->mt_ref_formulae INTO ls_ref_formula WHERE ref <> space.
      ...
      TRY.
          /nag/cl_excel_common=>convert_range2column_a_row( EXPORTING i_range        = ls_ref_formula-ref
          ...
          ls_referenced_cell-col_from = /nag/cl_excel_common=>convert_column2int( lv_col_from ).
          ls_referenced_cell-col_to   = /nag/cl_excel_common=>convert_column2int( lv_col_to ).


          CLEAR ls_referenced_cell-ref_cell.
          TRY.
              ls_referenced_cell-ref_cell(3) = /nag/cl_excel_common=>convert_column2alpha( ls_ref_formula-column ).
...
*--------------------------------------------------------------------*
    LOOP AT me->mt_ref_formulae INTO ls_ref_formula WHERE ref = space.
      ...
      TRY.
          lv_current_cell(3) = /nag/cl_excel_common=>convert_column2alpha( ls_ref_formula-column ).
      ...
      LOOP AT lts_referenced_cells INTO ls_referenced_cell WHERE sheet     = ls_ref_formula-sheet
        ...
        TRY.
            ...
            lv_resulting_formula = /nag/cl_excel_common=>determine_resulting_formula( iv_reference_cell     = ls_referenced_cell-ref_cell
  ...
  METHOD /nag/if_excel_reader~load.
*--------------------------------------------------------------------*
* ToDos:
*        2do§1   Map Document Properties to /NAG/CL_EXCEL
*--------------------------------------------------------------------*
...
* §1  Create EXCEL-Object we want to return to caller
*--------------------------------------------------------------------*
    IF iv_/nag/cl_excel_classname IS INITIAL.
      CREATE OBJECT r_excel.
    ELSE.
      CREATE OBJECT r_excel TYPE (iv_/nag/cl_excel_classname).
    ENDIF.
...
*--------------------------------------------------------------------*
  ...
  METHOD /nag/if_excel_reader~load_file.
...
*--------------------------------------------------------------------*
* Parse Excel data into /NAG/CL_EXCEL object from binary string
*--------------------------------------------------------------------*
    r_excel = /nag/if_excel_reader~load( i_excel2007            = lv_excel_data
                                     i_use_alternate_zip    = i_use_alternate_zip
                                     iv_/nag/cl_excel_classname = iv_/nag/cl_excel_classname ).
