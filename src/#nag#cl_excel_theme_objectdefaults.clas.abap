CLASS /nag/cl_excel_theme_objectdefaults DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS /nag/cl_excel_theme_objectdefaults IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF objectdefaults IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = /nag/cl_excel_theme=>c_theme_prefix
                                                                name   = /nag/cl_excel_theme=>c_theme_object_def
                                                                parent = lo_theme ).
    ELSE.
  ...
  METHOD load.
    objectdefaults = /nag/cl_excel_common=>clone_ixml_with_namespaces( io_object_def ).
  ENDMETHOD.                    "load
ENDCLASS.
