CLASS /nag/cl_excel_writer_xlsm DEFINITION
  PUBLIC
  INHERITING FROM /nag/cl_excel_writer_2007
  CREATE PUBLIC .

  PUBLIC SECTION.
*"* public components of class /NAG/CL_EXCEL_WRITER_XLSM
*"* do not include other source files here!!!
  PROTECTED SECTION.

*"* protected components of class /NAG/CL_EXCEL_WRITER_XLSM
*"* do not include other source files here!!!
    ...
    METHODS create_xl_workbook
        REDEFINITION .
  PRIVATE SECTION.
*"* private components of class /NAG/CL_EXCEL_WRITER_XLSM
*"* do not include other source files here!!!
ENDCLASS.



CLASS /nag/cl_excel_writer_xlsm IMPLEMENTATION.
...
* Add vbaProject.bin to zip
    io_zip->add( name    = me->c_xl_vbaproject
                 content = me->excel->/nag/if_excel_book_vba_project~vbaproject ).
...
* STEP 2: modify XML adding the vbaProject relation
    ...
    WHILE lo_element IS BOUND.
      lo_element->set_attribute_ns( name  = lc_xml_attr_codename
                                    value = io_worksheet->/nag/if_excel_sheet_vba_project~codename_pr ).
      lo_element ?= lo_iterator->get_next( ).
    ENDWHILE.
...
* STEP 2: modify XML adding the vbaProject relation
    ...
    WHILE lo_element IS BOUND.
      lo_element->set_attribute_ns( name  = lc_xml_attr_codename
                                    value = me->excel->/nag/if_excel_book_vba_project~codename ).
      lo_element ?= lo_iterator->get_next( ).
    ENDWHILE.
    ...
    WHILE lo_element IS BOUND.
      lo_element->set_attribute_ns( name  = lc_xml_attr_codename
                                    value = me->excel->/nag/if_excel_book_vba_project~codename_pr ).
      lo_element ?= lo_iterator->get_next( ).
    ENDWHILE.
