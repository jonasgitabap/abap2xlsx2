...
CLASS /nag/cl_excel_worksheet DEFINITION LOCAL FRIENDS
...
* ================
    DATA:
      f_cut TYPE REF TO /nag/cl_excel_worksheet.  "class under test
...
CLASS ltc_calculate_table_bottom_rig DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      ...
      field_catalog      TYPE /nag/excel_t_fieldcatalog,
      field_catalog_line TYPE /nag/excel_s_fieldcatalog,
      table_settings     TYPE /nag/excel_s_table_settings.  "class under test
...
CLASS ltc_check_cell_column_formula DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS: should_fail
      IMPORTING
        ip_formula_id TYPE /nag/excel_s_cell_data-column_formula_id
        ip_formula    TYPE /nag/excel_s_cell_data-cell_formula OPTIONAL
        ip_value      TYPE /nag/excel_s_cell_data-cell_value OPTIONAL
        ip_row        TYPE /nag/excel_s_cell_data-cell_row
        ip_column     TYPE /nag/excel_s_cell_data-cell_column
        ip_exp        TYPE string
      RAISING
        /nag/cx_excel.

    DATA: mt_column_formulas TYPE /nag/cl_excel_worksheet=>mty_th_column_formula,
          c_messages         LIKE /nag/cl_excel_worksheet=>c_messages.
...
CLASS ltc_check_overlapping DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    TYPES : BEGIN OF ty_parameters,
              BEGIN OF input,
                table_settings       TYPE /nag/excel_s_table_settings,
                other_table_settings TYPE /nag/cl_excel_worksheet=>ty_table_settings,
              END OF input,
              BEGIN OF output,
                fails TYPE abap_bool,
              END OF output,
            END OF ty_parameters.
    DATA: table_1_settings TYPE /nag/excel_s_table_settings.
...
CLASS ltc_normalize_column_heading DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      cut           TYPE REF TO /nag/cl_excel_worksheet,  "class under test
    ...
    METHODS:
      ...
      assert
        IMPORTING
          ...
          ip_colname_exp TYPE /nag/excel_column_name OPTIONAL,

      assert_multi
        IMPORTING
          it_fc          TYPE /nag/excel_t_fieldcatalog OPTIONAL
  ...
  PRIVATE SECTION.
    TYPES : BEGIN OF ty_parameters,
              BEGIN OF input,
                ...
                row          TYPE /nag/excel_cell_row,
                row_to       TYPE /nag/excel_cell_row,
              END OF input,
              BEGIN OF output,
                fails        TYPE abap_bool,
                column_start TYPE /nag/excel_cell_column,
                column_end   TYPE /nag/excel_cell_column,
...
CLASS ltc_normalize_style_param DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    TYPES : BEGIN OF ty_parameters_output,
              fails TYPE abap_bool,
              guid  TYPE /nag/excel_cell_style,
    ...
    METHODS setup.
    METHODS:
      ref_to_/nag/cl_excel_style FOR TESTING RAISING cx_static_check,
      /nag/excel_cell_style FOR TESTING RAISING cx_static_check,
...
* =============
    ...
    DATA lo_excel TYPE REF TO /nag/cl_excel.
...
CLASS ltc_calculate_table_bottom_rig IMPLEMENTATION.
  ...
  METHOD simple.
    ...
    /nag/cl_excel_worksheet=>calculate_table_bottom_right(
      EXPORTING
        ip_table         = test_table
...
CLASS ltc_check_cell_column_formula IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    c_messages = /nag/cl_excel_worksheet=>c_messages.
  ...
  METHOD success.
    ...
    /nag/cl_excel_worksheet=>check_cell_column_formula(
  ...
  METHOD should_fail.
    ...
    DATA: lo_exception TYPE REF TO /nag/cx_excel.
    ...
    TRY.
      ...
      CATCH /nag/cx_excel INTO lo_exception.
...
CLASS ltc_check_overlapping IMPLEMENTATION.
  ...
  METHOD assert.
    DATA: error TYPE REF TO /nag/cx_excel.
    FIELD-SYMBOLS:
          <table> TYPE STANDARD TABLE.

    TRY.

        /nag/cl_excel_worksheet=>check_table_overlapping(
...
CLASS ltc_normalize_column_heading IMPLEMENTATION.
  ...
  METHOD default_text_if_none.
    ...
    CONSTANTS: lc_initial_line TYPE /nag/excel_s_fieldcatalog VALUE IS INITIAL.
    DATA lt_fc TYPE /nag/excel_t_fieldcatalog.
  ...
  METHOD assert.
    ...
    DATA:
      ls_fc TYPE /nag/excel_s_fieldcatalog,
      lt_fc TYPE /nag/excel_t_fieldcatalog.
  ...
  METHOD setup.
    ...
    DATA:
      lo_ex      TYPE REF TO /nag/cx_excel,
...
CLASS ltc_normalize_columnrow_param IMPLEMENTATION.
  ...
  METHOD assert.
    DATA: act        TYPE ty_parameters-output,
          error      TYPE REF TO /nag/cx_excel,
...
CLASS ltc_normalize_style_param IMPLEMENTATION.
  ...
  METHOD ref_to_/nag/cl_excel_style.
  ...
  METHOD assert.
    ...
    TRY.
        ...
        IF exp-fails = abap_true.
          message = |Input type { input_text } accepted but only REF TO \\CLASS=/NAG/CL_EXCEL_STYLE, \\TYPE=/NAG/EXCEL_CELL_STYLE or any X type are supported|.
          cl_abap_unit_assert=>fail( msg = message ).
        ENDIF.
