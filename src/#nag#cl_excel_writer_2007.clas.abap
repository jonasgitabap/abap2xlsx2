CLASS /nag/cl_excel_writer_2007 DEFINITION
  PUBLIC
  CREATE PUBLIC .

  PUBLIC SECTION.

*"* public components of class /NAG/CL_EXCEL_WRITER_2007
*"* do not include other source files here!!!
    INTERFACES /nag/if_excel_writer .
    METHODS constructor.

  PROTECTED SECTION.

*"* protected components of class /NAG/CL_EXCEL_WRITER_2007
*"* do not include other source files here!!!
    TYPES: BEGIN OF mty_column_formula_used,
             id TYPE /nag/excel_s_cell_data-column_formula_id,
             si TYPE string,
             "! type: shared, etc.
             t  TYPE string,
    ...
    DATA excel TYPE REF TO /nag/cl_excel .
    DATA shared_strings TYPE /nag/excel_t_shared_string .
    DATA styles_cond_mapping TYPE /nag/excel_t_styles_cond_mapping .
    DATA styles_mapping TYPE /nag/excel_t_styles_mapping .
    ...
    METHODS create_dxf_style
      IMPORTING
        !iv_cell_style    TYPE /nag/excel_cell_style
        !io_dxf_element   TYPE REF TO if_ixml_element
        !io_ixml_document TYPE REF TO if_ixml_document
        !it_cellxfs       TYPE /nag/excel_t_cellxfs
        !it_fonts         TYPE /nag/excel_t_style_font
        !it_fills         TYPE /nag/excel_t_style_fill
      CHANGING
        !cv_dfx_count     TYPE i .
    ...
    METHODS create_xl_charts
      IMPORTING
        !io_drawing       TYPE REF TO /nag/cl_excel_drawing
      RETURNING
        VALUE(ep_content) TYPE xstring .
    ...
    METHODS create_xl_styles_font_node
      IMPORTING
        !io_document TYPE REF TO if_ixml_document
        !io_parent   TYPE REF TO if_ixml_element
        !is_font     TYPE /nag/excel_s_style_font
        !iv_use_rtf  TYPE abap_bool DEFAULT abap_false .
    METHODS create_xl_table
      IMPORTING
        !io_table         TYPE REF TO /nag/cl_excel_table
      RETURNING
        VALUE(ep_content) TYPE xstring
      RAISING
        /nag/cx_excel .
    ...
    METHODS get_shared_string_index
      IMPORTING
        !ip_cell_value  TYPE /nag/excel_cell_value
    ...
    METHODS escape_string_value
      IMPORTING
        !iv_value     TYPE /nag/excel_cell_value
      RETURNING
        VALUE(result) TYPE /nag/excel_cell_value.
    METHODS set_vml_shape_footer
      IMPORTING
        !is_footer        TYPE /nag/excel_s_worksheet_head_foot
      RETURNING
        VALUE(ep_content) TYPE string .
    METHODS set_vml_shape_header
      IMPORTING
        !is_header        TYPE /nag/excel_s_worksheet_head_foot
      RETURNING
        VALUE(ep_content) TYPE string .
    ...
    METHODS create_xl_sheet_column_formula
      IMPORTING
        io_document             TYPE REF TO if_ixml_document
        it_column_formulas      TYPE /nag/cl_excel_worksheet=>mty_th_column_formula
        is_sheet_content        TYPE /nag/excel_s_cell_data
      EXPORTING
...
*"* private components of class /NAG/CL_EXCEL_WRITER_2007
*"* do not include other source files here!!!
...
CLASS /nag/cl_excel_writer_2007 IMPLEMENTATION.
...
* Office 2007 file format is a cab of several xml files with extension .xlsx
    ...
    DATA: lo_zip              TYPE REF TO cl_abap_zip,
          ...
          lo_active_worksheet TYPE REF TO /nag/cl_excel_worksheet,
          lo_iterator         TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_nested_iterator  TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_table            TYPE REF TO /nag/cl_excel_table,
          lo_drawing          TYPE REF TO /nag/cl_excel_drawing,
          lo_drawings         TYPE REF TO /nag/cl_excel_drawings,
          lo_comment          TYPE REF TO /nag/cl_excel_comment,   " (+) Issue #180
          lo_comments         TYPE REF TO /nag/cl_excel_comments.  " (+) Issue #180
...
* Add Header/Footer image
      DATA: lt_drawings TYPE /nag/excel_t_drawings.
...
**********************************************************************
* STEP 11: Add media
    lo_iterator = me->excel->get_drawings_iterator( /nag/cl_excel_drawing=>type_image ).
    WHILE lo_iterator->has_next( ) EQ abap_true.
...
**********************************************************************
* STEP 12: Add charts
    lo_iterator = me->excel->get_drawings_iterator( /nag/cl_excel_drawing=>type_chart ).
    WHILE lo_iterator->has_next( ) EQ abap_true.
      lo_drawing ?= lo_iterator->get_next( ).
...
* Begin - Add - GGAR
      " Comments
      DATA: lo_comments TYPE REF TO /nag/cl_excel_comments.
...
* End   - Add - GGAR
      ...
      " Drawings
      DATA: lo_drawings TYPE REF TO /nag/cl_excel_drawings.
...
* STEP 4: Create subnodes
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_application
                                                     parent = lo_document ).
    lv_value = excel->/nag/if_excel_book_properties~application.
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_docsecurity
                                                              parent = lo_document ).
    lv_value = excel->/nag/if_excel_book_properties~docsecurity.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_scalecrop
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->/nag/if_excel_book_properties~scalecrop ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    " Company
    IF excel->/nag/if_excel_book_properties~company IS NOT INITIAL.
      lo_element = lo_document->create_simple_element( name   = lc_xml_node_company
                                                       parent = lo_document ).
      lv_value = excel->/nag/if_excel_book_properties~company.
      lo_element->set_value( value = lv_value ).
      lo_element_root->append_child( new_child = lo_element ).
    ENDIF.
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_linksuptodate
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->/nag/if_excel_book_properties~linksuptodate ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_shareddoc
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->/nag/if_excel_book_properties~shareddoc ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_hyperlinkschanged
                                                     parent = lo_document ).
    lv_value = me->flag2bool( excel->/nag/if_excel_book_properties~hyperlinkschanged ).
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element( name   = lc_xml_node_appversion
                                                     parent = lo_document ).
    lv_value = excel->/nag/if_excel_book_properties~appversion.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
...
* STEP 4: Create subnodes
    ...
    lo_element = lo_document->create_simple_element_ns( name   = lc_xml_node_creator
                                                        prefix = lc_dc_ns
                                                        parent = lo_document ).
    lv_value = excel->/nag/if_excel_book_properties~creator.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element_ns( name   = lc_xml_node_description
                                                        prefix = lc_dc_ns
                                                        parent = lo_document ).
    lv_value = excel->/nag/if_excel_book_properties~description.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    lo_element = lo_document->create_simple_element_ns( name   = lc_xml_node_lastmodifiedby
                                                        prefix = lc_cp_ns
                                                        parent = lo_document ).
    lv_value = excel->/nag/if_excel_book_properties~lastmodifiedby.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
    ...
    CONVERT TIME STAMP excel->/nag/if_excel_book_properties~created TIME ZONE sy-zonlo INTO DATE lv_date TIME lv_time.
    ...
    lo_element->set_attribute_ns( name    = lc_xml_attr_type
                                  prefix  = lc_xsi_ns
                                  value   = lc_xml_attr_target ).
    CONVERT TIME STAMP excel->/nag/if_excel_book_properties~modified TIME ZONE sy-zonlo INTO DATE lv_date TIME lv_time.
    CONCATENATE lv_date lv_time INTO lv_value RESPECTING BLANKS.
    REPLACE ALL OCCURRENCES OF REGEX '([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})' IN lv_value WITH '$1-$2-$3T$4:$5:$6Z'.
    lo_element->set_value( value = lv_value ).
    lo_element_root->append_child( new_child = lo_element ).
...
* STEP 5: Create xstring stream
  ...
  METHOD create_dxf_style.
    ...
    DATA: ls_styles_mapping     TYPE /nag/excel_s_styles_mapping,
          ls_cellxfs            TYPE /nag/excel_s_cellxfs,
          ls_style_cond_mapping TYPE /nag/excel_s_styles_cond_mapping,
          lo_sub_element        TYPE REF TO if_ixml_element,
          lo_sub_element_2      TYPE REF TO if_ixml_element,
          lv_index              TYPE i,
          ls_font               TYPE /nag/excel_s_style_font,
          lo_element_font       TYPE REF TO if_ixml_element,
          lv_value              TYPE string,
          ls_fill               TYPE /nag/excel_s_style_fill,
    ...
    ELSE.
      ...
      IF ls_fill IS NOT INITIAL.
        ...
        IF  ls_fill-fgcolor-rgb IS INITIAL AND
          ls_fill-fgcolor-indexed EQ /nag/cl_excel_style_color=>c_indexed_not_set AND
          ls_fill-fgcolor-theme EQ /nag/cl_excel_style_color=>c_theme_not_set AND
          ls_fill-fgcolor-tint IS INITIAL AND ls_fill-bgcolor-indexed EQ /nag/cl_excel_style_color=>c_indexed_sys_foreground.
...
* STEP 4: Create chart
    ...
    DATA lo_chartb TYPE REF TO /nag/cl_excel_graph_bars.
    DATA lo_chartp TYPE REF TO /nag/cl_excel_graph_pie.
    DATA lo_chartl TYPE REF TO /nag/cl_excel_graph_line.
    DATA lo_chart TYPE REF TO /nag/cl_excel_graph.

    DATA ls_serie TYPE /nag/cl_excel_graph=>s_series.
    DATA ls_ax TYPE /nag/cl_excel_graph_bars=>s_ax.
    DATA lv_str TYPE string.

    "Identify chart type
    CASE io_drawing->graph_type.
      WHEN /nag/cl_excel_drawing=>c_graph_bars.
        lo_chartb ?= io_drawing->graph.
      WHEN /nag/cl_excel_drawing=>c_graph_pie.
        lo_chartp ?= io_drawing->graph.
      WHEN /nag/cl_excel_drawing=>c_graph_line.
    ...
    CASE io_drawing->graph_type.
      WHEN /nag/cl_excel_drawing=>c_graph_bars.
        ...
        ENDLOOP.
        "endseries
        IF lo_chartb->ns_groupingval = /nag/cl_excel_graph_bars=>c_groupingval_stacked.
        ...
        LOOP AT lo_chartb->axes INTO ls_ax.
          ...
          lo_element4->set_attribute_ns( name  = 'val'
                                  value = ls_ax-axid ).
          CASE ls_ax-type.
            WHEN /nag/cl_excel_graph_bars=>c_catax.
            ...
            WHEN /nag/cl_excel_graph_bars=>c_valax.
      ...
      WHEN /nag/cl_excel_drawing=>c_graph_line.
        ...
        LOOP AT lo_chartl->axes INTO ls_ax.
          ...
          lo_element4->set_attribute_ns( name  = 'val'
                                  value = ls_ax-axid ).
          CASE ls_ax-type.
            WHEN /nag/cl_excel_graph_line=>c_catax.
              lo_element3 = lo_document->create_simple_element( name = lc_xml_node_catax
                                                     parent = lo_element2 ).
              lo_element4 = lo_document->create_simple_element( name = lc_xml_node_axid
                                                     parent = lo_element3 ).
              ...
              lo_element4->set_attribute_ns( name  = 'val'
                                             value = ls_ax-nomultilvllbl ).
            WHEN /nag/cl_excel_graph_line=>c_valax.
              lo_element3 = lo_document->create_simple_element( name = lc_xml_node_valax
                                                     parent = lo_element2 ).
              lo_element4 = lo_document->create_simple_element( name = lc_xml_node_axid
                                                     parent = lo_element3 ).
...
*       lo_element_charset     TYPE REF TO if_ixml_element,
          ...
          lo_iterator            TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_comments            TYPE REF TO /nag/cl_excel_comments,
...
** Constant node name
    ...
    DATA: lo_drawing      TYPE REF TO /nag/cl_excel_drawing,
          ...
          lt_drawings     TYPE /nag/excel_t_drawings.

    FIELD-SYMBOLS: <fs_temp>     TYPE sstrtable,
                   <fs_drawings> TYPE /nag/excel_s_drawings.
...
** Constant node name
    ...
    DATA: lo_iterator     TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_drawing      TYPE REF TO /nag/cl_excel_drawing,
          lo_document     TYPE REF TO if_ixml_document,
          lo_element_root TYPE REF TO if_ixml_element,
          lo_element      TYPE REF TO if_ixml_element,
...
* STEP 4: Create subnodes
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      lo_drawing ?= lo_iterator->get_next( ).
      IF lo_drawing->get_type( ) = /nag/cl_excel_drawing=>type_image_header_footer.
        ADD 1 TO lv_relation_id.
...
** Constant node name
    ...
    DATA: lo_element_graphicframe TYPE REF TO if_ixml_element,
          ...
          ls_position             TYPE /nag/excel_drawing_position,
          lv_col                  TYPE string, " /nag/excel_cell_column,
          lv_row                  TYPE string, " /nag/excel_cell_row.
...
** Constant node name
    ...
    DATA: lo_document              TYPE REF TO if_ixml_document,
          ...
          lv_str_column            TYPE /nag/excel_cell_column_alpha,
...
* STEP 4: Create v:shapetype
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      ...
      /nag/cl_excel_common=>convert_columnrow2column_a_row( EXPORTING i_columnrow = lo_comment->get_ref( )
                                                        IMPORTING e_column = lv_str_column
                                                                  e_row    = lv_row ).
      lv_column = /nag/cl_excel_common=>convert_column2int( lv_str_column ).
...
** Constant node name
    ...
    DATA: lt_cell_data       TYPE /nag/excel_t_cell_data_unsorted,
          lt_cell_data_rtf   TYPE /nag/excel_t_cell_data_unsorted,
          lv_value           TYPE string,
          ls_shared_string   TYPE /nag/excel_s_shared_string,
    ...
    FIELD-SYMBOLS: <fs_sheet_content> TYPE /nag/excel_s_cell_data,
                   <fs_rtf>           TYPE /nag/excel_s_rtf,
                   <fs_sheet_string>  TYPE /nag/excel_s_shared_string.
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_sheet_column_formula.
    ...
    TYPES: ls_column_formula_used     TYPE mty_column_formula_used,
           ...
           lv_top_cell_coords         TYPE /nag/excel_cell_coords,
           lv_bottom_cell_coords      TYPE /nag/excel_cell_coords,
           lv_cell_coords             TYPE /nag/excel_cell_coords,
    ...
    DATA: lv_value                   TYPE string,
          ...
          lv_ref_value               TYPE string,
          lv_1st_line_shared_formula TYPE abap_bool.
    FIELD-SYMBOLS: <ls_column_formula>      TYPE /nag/cl_excel_worksheet=>mty_s_column_formula,
                   <ls_column_formula_used> TYPE mty_column_formula_used.
    ...
    IF lv_1st_line_shared_formula = abap_true OR <ls_column_formula_used>-t <> 'shared'.
      lv_column_alpha = /nag/cl_excel_common=>convert_column2alpha( ip_column = is_sheet_content-cell_column ).
      ...
      ELSE.
        ...
        lv_value = /nag/cl_excel_common=>shift_formula(
  ...
  METHOD create_xl_sheet_ignored_errors.
    ...
    FIELD-SYMBOLS: <ls_ignored_errors> TYPE /nag/cl_excel_worksheet=>mty_s_ignored_errors.
  ...
  METHOD create_xl_sheet_pagebreaks.
    DATA: lo_pagebreaks     TYPE REF TO /nag/cl_excel_worksheet_pagebreaks,
          lt_pagebreaks     TYPE /nag/cl_excel_worksheet_pagebreaks=>tt_pagebreak_at,
...
** Constant node name
    ...
    DATA: lo_document     TYPE REF TO if_ixml_document,
          ...
          lo_iterator     TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_table        TYPE REF TO /nag/cl_excel_table,
          lo_link         TYPE REF TO /nag/cl_excel_hyperlink.
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_sheet_sheet_data.
    ...
    CONSTANTS: lc_dummy_cell_content       TYPE /nag/excel_s_cell_data-cell_value VALUE '})~~~ This is a dummy value for ABAP2XLSX and you should never find this in a real excelsheet Ihope'.
    ...
    DATA: col_count              TYPE int4,
          ...
          lt_values              TYPE /nag/excel_t_autofilter_values,
          ls_values              TYPE /nag/excel_s_autofilter_values,
...
*        lts_row_dimensions     TYPE /nag/excel_t_worksheet_rowdimensio,
          lo_row_iterator        TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_row                 TYPE REF TO /nag/cl_excel_row,
          lo_row_empty           TYPE REF TO /nag/cl_excel_row,
          lts_row_outlines       TYPE /nag/cl_excel_worksheet=>mty_ts_outlines_row,

          ls_last_row            TYPE /nag/excel_s_cell_data,
          ls_style_mapping       TYPE /nag/excel_s_styles_mapping,
...
*Build table to hold all table-areas attached to this sheet
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      lo_table ?= lo_iterator->get_next( ).
      ls_table_area-left   = /nag/cl_excel_common=>convert_column2int( lo_table->settings-top_left_column ).
...
*--------------------------------------------------------------------*
          lv_style_guid = io_worksheet->/nag/if_excel_sheet_properties~get_style( ).
...
** Constant node name
    ...
    DATA: lo_document        TYPE REF TO if_ixml_document,
          ...
          lo_sub_element_2   TYPE REF TO if_ixml_element,
          lo_iterator        TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_iterator2       TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_worksheet       TYPE REF TO /nag/cl_excel_worksheet,
          lo_style_cond      TYPE REF TO /nag/cl_excel_style_cond,
          lo_style           TYPE REF TO /nag/cl_excel_style.


    DATA: lt_fonts          TYPE /nag/excel_t_style_font,
          ls_font           TYPE /nag/excel_s_style_font,
          lt_fills          TYPE /nag/excel_t_style_fill,
          ls_fill           TYPE /nag/excel_s_style_fill,
          lt_borders        TYPE /nag/excel_t_style_border,
          ls_border         TYPE /nag/excel_s_style_border,
          lt_numfmts        TYPE /nag/excel_t_style_numfmt,
          ls_numfmt         TYPE /nag/excel_s_style_numfmt,
          lt_protections    TYPE /nag/excel_t_style_protection,
          ls_protection     TYPE /nag/excel_s_style_protection,
          lt_alignments     TYPE /nag/excel_t_style_alignment,
          ls_alignment      TYPE /nag/excel_s_style_alignment,
          lt_cellxfs        TYPE /nag/excel_t_cellxfs,
          ls_cellxfs        TYPE /nag/excel_s_cellxfs,
          ls_styles_mapping TYPE /nag/excel_s_styles_mapping,
          lt_colors         TYPE /nag/excel_t_style_color_argb,
    ...
    TYPES: BEGIN OF ts_built_in_format,
             num_format TYPE /nag/excel_number_format,
             id         TYPE i,
    ...
    FIELD-SYMBOLS: <ls_built_in_format> LIKE LINE OF lt_built_in_num_formats,
                   <ls_reader_built_in> LIKE LINE OF /nag/cl_excel_style_number_format=>mt_built_in_num_formats.
...
* Prepare built-in number formats.
    LOOP AT /nag/cl_excel_style_number_format=>mt_built_in_num_formats ASSIGNING <ls_reader_built_in>.
...
* Compress number formats
      ...
      "-----------
      IF ls_numfmt-numfmt NE /nag/cl_excel_style_number_format=>c_format_date_std." and ls_numfmt-NUMFMT ne 'STD_NDEC'. " ALE Changes on going
        "---
...
* Use these instead of duplicating the entries here, since they seem to be language-dependant and adjust to user settings in excel
          ...
          ELSE.
            ...
            ENDIF.
            ADD /nag/cl_excel_common=>c_excel_numfmt_offset TO ls_cellxfs-numfmtid. " Add OXML offset for custom styles
          ENDIF.
      ...
      ELSE.
        ls_cellxfs-applynumberformat    = 1.
        IF ls_numfmt-numfmt EQ /nag/cl_excel_style_number_format=>c_format_date_std.
...
* Remap styles
    ...
    LOOP AT lt_numfmts INTO ls_numfmt.
      lo_element_numfmt = lo_document->create_simple_element( name   = lc_xml_node_numfmt
                                                              parent = lo_document ).
      lv_value = sy-tabix + /nag/cl_excel_common=>c_excel_numfmt_offset.
      CONDENSE lv_value.
...
* begin of change issue #366 - missing conditional rules: top10, move dfx-styles to own method
    ...
    lo_element->set_attribute_ns( name  = lc_xml_attr_defaulttablestyle
                                  value = /nag/cl_excel_table=>builtinstyle_medium9 ).
    lo_element->set_attribute_ns( name  = lc_xml_attr_defaultpivotstyle
                                  value = /nag/cl_excel_table=>builtinstyle_pivot_light16 ).
    lo_element_root->append_child( new_child = lo_element ).
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_styles_color_node.
    ...
    CHECK is_color-rgb IS NOT INITIAL OR
          is_color-indexed <> /nag/cl_excel_style_color=>c_indexed_not_set OR
          is_color-theme <> /nag/cl_excel_style_color=>c_theme_not_set OR
    ...
    IF is_color-indexed <> /nag/cl_excel_style_color=>c_indexed_not_set.
      lv_value = /nag/cl_excel_common=>number_to_excel_string( is_color-indexed ).
      lo_sub_element->set_attribute_ns( name  = lc_xml_attr_indexed
                                        value = lv_value ).
    ENDIF.

    IF is_color-theme <> /nag/cl_excel_style_color=>c_theme_not_set.
      lv_value = /nag/cl_excel_common=>number_to_excel_string( is_color-theme ).
      lo_sub_element->set_attribute_ns( name  = lc_xml_attr_theme
                                        value = lv_value ).
    ENDIF.

    IF is_color-tint IS NOT INITIAL.
      lv_value = /nag/cl_excel_common=>number_to_excel_string( is_color-tint ).
  ...
  METHOD create_xl_table.
    ...
    DATA: lo_document     TYPE REF TO if_ixml_document,
          ...
          ls_fieldcat     TYPE /nag/excel_s_fieldcatalog.
...
* STEP 3: Create main node table
    ...
    lv_value = /nag/cl_excel_common=>number_to_excel_string( ip_value = lv_id ).
    ...
    IF io_table->settings-table_name IS NOT INITIAL AND lv_match EQ 0.
      ...
      IF ( strlen( io_table->settings-table_name ) = 1 AND io_table->settings-table_name CO 'CcRr' )
         OR /nag/cl_excel_common=>shift_formula(
...
* STEP 5: Create xstring stream
  ...
  METHOD create_xl_theme.
    DATA: lo_theme TYPE REF TO /nag/cl_excel_theme.
...
** Constant node name
    ...
    DATA: lo_document       TYPE REF TO if_ixml_document,
          ...
          lo_sub_element    TYPE REF TO if_ixml_element,
          lo_iterator       TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_iterator_range TYPE REF TO /nag/cl_excel_collection_iterator,
          lo_worksheet      TYPE REF TO /nag/cl_excel_worksheet,
    ...
    DATA: lv_xml_node_ridx_id TYPE string,
          lv_value            TYPE string,
          lv_syindex          TYPE string,
          lv_active_sheet     TYPE /nag/excel_active_worksheet.
...
* STEP 4: Create subnode
    ...
    IF me->excel->/nag/if_excel_book_protection~protected EQ abap_true.
      lo_element = lo_document->create_simple_element( name   = lc_xml_node_workbookprotection
                                                       parent = lo_document ).
      lv_value = me->excel->/nag/if_excel_book_protection~workbookpassword.
      IF lv_value IS NOT INITIAL.
        lo_element->set_attribute_ns( name  = lc_xml_attr_workbookpassword
                                      value = lv_value ).
      ENDIF.
      lv_value = me->excel->/nag/if_excel_book_protection~revisionspassword.
      IF lv_value IS NOT INITIAL.
        lo_element->set_attribute_ns( name  = lc_xml_attr_revisionspassword
                                      value = lv_value ).
      ENDIF.
      lv_value = me->excel->/nag/if_excel_book_protection~lockrevision.
      CONDENSE lv_value NO-GAPS.
      lo_element->set_attribute_ns( name  = lc_xml_attr_lockrevision
                                    value = lv_value ).
      lv_value = me->excel->/nag/if_excel_book_protection~lockstructure.
      CONDENSE lv_value NO-GAPS.
      lo_element->set_attribute_ns( name  = lc_xml_attr_lockstructure
                                    value = lv_value ).
      lv_value = me->excel->/nag/if_excel_book_protection~lockwindows.
      CONDENSE lv_value NO-GAPS.
...
* issue #365 - test if sheet exists - otherwise set active worksheet to 1
    ...
    WHILE lo_iterator->has_next( ) EQ abap_true.
      ...
      lo_sub_element->set_attribute_ns( name  = lc_xml_attr_sheetid
                                        value = lv_syindex ).
      IF lo_worksheet->/nag/if_excel_sheet_properties~hidden EQ /nag/if_excel_sheet_properties=>c_hidden.
        lo_sub_element->set_attribute_ns( name  = lc_xml_attr_state
                                          value = 'hidden' ).
      ELSEIF lo_worksheet->/nag/if_excel_sheet_properties~hidden EQ /nag/if_excel_sheet_properties=>c_veryhidden.
        lo_sub_element->set_attribute_ns( name  = lc_xml_attr_state
                                          value = 'veryHidden' ).
      ENDIF.
...
* STEP 5: Create xstring stream
  ...
  METHOD get_shared_string_index.
    ...
    DATA ls_shared_string TYPE /nag/excel_s_shared_string.
  ...
  METHOD is_formula_shareable.
    ...
    IF ip_formula NA '!'.
      lv_test_shared = /nag/cl_excel_common=>shift_formula(
  ...
  METHOD set_vml_shape_footer.
    ...
    DATA: lv_content_left         TYPE string,
          ...
          lv_value                TYPE string,
          ls_drawing_position     TYPE /nag/excel_drawing_position.
...
* BODY
  ...
  METHOD /nag/if_excel_writer~write_file.
    me->excel = io_excel.
